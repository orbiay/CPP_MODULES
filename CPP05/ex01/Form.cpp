#include"Form.hpp"
Form::Form(): name("no value"),grade_to_sign(0),grade_to_exec(0){
    is_sign = 0;
}
Form::~Form(){
    
}
Form::Form(const Form& obj):name(obj.name),grade_to_sign(obj.grade_to_sign),grade_to_exec(obj.grade_to_exec){
    is_sign = obj.is_sign;
}
