...
/*   Updated: 2022/09/07 19:56:00 by orbiay           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void information(PhoneBook person , int i)
{
	...
	if (person.contacts[i].firstname.size() > 10)
		std :: cout << " | "  << std::setw(13) << person.contacts[i].firstname.substr(0,9) + ".";
	else
		std :: cout << " | "  << std::setw(13) << person.contacts[i].firstname;
	if (person.contacts[i].lastname.size() > 10)
		std :: cout << " | "  << std::setw(13) << person.contacts[i].lastname.substr(0,9) + ".";
	else
		std :: cout << " | "  << std::setw(13) << person.contacts[i].lastname;
	if (person.contacts[i].nickname.size() > 10)
		std :: cout << " | "  << std::setw(13) << person.contacts[i].nickname.substr(0,9) + ".";
	else
		std :: cout << " | "  << std::setw(13) << person.contacts[i].nickname;
	if (person.contacts[i].phonenumber.size() > 10)
		std :: cout << " | "  << std::setw(13) << person.contacts[i].phonenumber.substr(0,9) + ".";
	else
		std :: cout << " | "  << std::setw(13) << person.contacts[i].phonenumber;
	if (person.contacts[i].darkestsecret.size() > 10)
		std :: cout << " | "  << std::setw(13) << person.contacts[i].darkestsecret.substr(0,9) + ".";
	else
		std :: cout << " | "  << std::setw(13) << person.contacts[i].darkestsecret;
}
...
void search(PhoneBook person)
{
	...
	getline (std::cin,str);
	if (str.empty() || !is_digits(str.c_str())){
		std::cout<<"Check your input" <<std::endl;
		return;
	}
	if (std::stoi(str.c_str()) >= i)
		std :: cout << "Sorry We Don't Have Any Information About This Contact ." << std :: endl;
	else
	{
		std :: cout << str << " | " << std::setw(10) << person.contacts[std::stoi(str.c_str())].firstname << std::setw(10) 
		<< person.contacts[std::stoi(str.c_str())].lastname << std::setw(10) << person.contacts[std::stoi(str.c_str())].nickname << "\n";
	}
}
